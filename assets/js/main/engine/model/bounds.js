...
export default class Bounds {
    ...
    getRandomPointWithPadding(padding) {
        const rng = Rng.getInstance();
        const minWithPadding = this.min.addScalar(padding);
        const maxWithPadding = this.max.subtractScalar(padding);
        return new Vector2(rng.getRandomInt(minWithPadding.x, maxWithPadding.x), rng.getRandomInt(minWithPadding.y, maxWithPadding.y));
    }
}
